...
# spTransform(gr_shape_14,CRS(proj4string(crop_rast_14)))
#
...
gr_shape_14@data$sowing_met = c("Water")
crop_rast_14 = crop(gr_rast_14, extent(gr_grid_25_sinu))
gr_shape_14@data$sowing_met[which(is.na(gr_shape_14@data$sowing_met))] = 'Unknown'
...
# prodry_142k = ZonalPipe(gr_grid_sinu,raster_obs_14_dry,rast_grid, 'pippo',
#                       'int_id', stat="mean", cut = F)
# 
# prodry_142k_count = ZonalPipe(gr_grid_sinu,raster_obs_14_dry,rast_grid, 'pippo',
#                             'int_id', stat="countna", cut = F)

prowat_142k = ZonalPipe(gr_grid_sinu,raster_obs_wat_14,rast_grid, 'pippo',
                      'int_id', stat="mean", cut = F)

prowatcount_142k = ZonalPipe(gr_grid_sinu,raster_obs_wat_14,rast_grid, 'pippo',
                           'int_id', stat="countna", cut = F)
...
#
gr_grid_2k = readOGR(dsn = dirname(gr_grid_reduced),layer = basename(file_path_sans_ext(gr_grid_reduced)))
gr_grid_25_sinu = spTransform(gr_grid_2k,CRS(proj4string(gr_rast_15)))
writeOGR(gr_grid_25_sinu, dsn = Accessory_folder,layer  ='grid_2k_sinu', driver = 'ESRI Shapefile',overwrite_layer = T)

gr_shape_15@data$sowing_met = c("Water")
...
# prodry_152k = ZonalPipe(gr_grid_sinu,raster_obs_15_dry,rast_grid, 'pippo',
#                         'int_id', stat="mean", cut = F)
# 
# prodry_152k_count = ZonalPipe(gr_grid_sinu,raster_obs_15_dry,rast_grid, 'pippo',
#                               'int_id', stat="countna", cut = F)

prowat_152k = ZonalPipe(gr_grid_sinu,raster_obs_wat_15,rast_grid, 'pippo',
                        'int_id', stat="mean", cut = F)

prowatcount_152k = ZonalPipe(gr_grid_sinu,raster_obs_wat_15,rast_grid, 'pippo',
                             'int_id', stat="countna", cut = F)
...
# spTransform(gr_shape_16,CRS(proj4string(crop_rast_16)))
#
gr_grid_2k = readOGR(dsn = dirname(gr_grid_reduced),layer = basename(file_path_sans_ext(gr_grid_reduced)))
gr_grid_25_sinu = spTransform(gr_grid_2k,CRS(proj4string(gr_rast_16)))
writeOGR(gr_grid_25_sinu, dsn = Accessory_folder,layer  ='grid_2k_sinu', driver = 'ESRI Shapefile',overwrite_layer = T)

gr_shape_16@data$sowing_met = c("Water")
crop_rast_16 = crop(gr_rast_16, extent(gr_grid_25_sinu))
gr_shape_16@data$sowing_met[which(is.na(gr_shape_16@data$sowing_met))] = 'Unknown'
# gr_shape_16@data$sowing_met[which(gr_shape_16@data$sowing_met == 'Unknown' & gr_shape_16@data$sowing_doy< 120)] = 'Dry'
# gr_shape_16@data$sowing_met[which(gr_shape_16@data$sowing_met == 'Unknown' & gr_shape_16@data$sowing_doy >= 120)] = 'Water'
...
pro_162k = ZonalPipe(gr_grid_sinu,crop_rast_16,rast_grid,'pippo',
                     'int_id', stat="mean", cut = T)

# prodry_162k = ZonalPipe(gr_grid_sinu,raster_obs_16_dry,rast_grid, 'pippo',
#                         'int_id', stat="mean", cut = F)
# 
# prodry_162k_count = ZonalPipe(gr_grid_sinu,raster_obs_16_dry,rast_grid, 'pippo',
#                               'int_id', stat="countna", cut = F)

prowat_162k = ZonalPipe(gr_grid_sinu,raster_obs_wat_16,rast_grid, 'pippo',
                        'int_id', stat="mean", cut = F)

prowatcount_162k = ZonalPipe(gr_grid_sinu,raster_obs_wat_16,rast_grid, 'pippo',
                             'int_id', stat="countna", cut = F)

proall_162k = ZonalPipe(gr_grid_sinu,raster_obs_16,rast_grid, 'pippo',
                        'int_id', stat="mean", cut = F)

proallcount_162k = ZonalPipe(gr_grid_sinu,raster_obs_16,rast_grid, 'pippo',
                             'int_id', stat="countna", cut = F)                             
...
gr_2k = profull_14_15_162k
gr_2k_melt  = protot_melt_14_15_162k
gr_stats_14_15_16 = ddply(subset(gr_2k_melt),.(variable) ,summarize, count = length(value),avg = mean(value, na.rm = T), stdev = sd(value, na.rm = T), mae = mean(abs(value),na.rm = T))
save(gr_2k,gr_2k_melt, gr_stats_14_15_16,file = file.path(out_folder,'GR_val_2k.RData'))
