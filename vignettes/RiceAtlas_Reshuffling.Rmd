...
in_data <- data.table::fread("/home/lb/my_data/prasia/Data/vector/NormalizedDB.txt")
head(in_data)
```
...
We also shows a very "anomalous" high frequency at `Peak - Start = 30`, but we can 
...
ggplot(data = in_phl, aes(x = Sub_region)) + theme_bw() +
  ...
  facet_wrap(~Season, scales = "free_x") + coord_flip()  +
...
out_atlas <- out_atlas %>% 
  ...
  lbscripts::mutate_when((Start == 213 & Peak == 227 & End == 213), list(End = NA)) %>% 
   #IND Orissa Nuapada
  ...
  lbscripts::mutate_when((peakstart_lgt == -14), list(Start = NA)) %>% 
  #several places with "30   -5   35" --> Remove Start
  lbscripts::mutate_when((peakstart_lgt == -5),                         list(Start   = NA)) %>% 
    # re-create the diffs for test purposes
  dplyr::mutate(endstart_lgt = (End - Start),
                peakstart_lgt = (Peak - Start), 
                endpeak_lgt   = (End - Peak)) 
...
write.table(final_out,
           file = "/home/lb/my_data/prasia/Data/vector/NormalizedDB_Asia_final.txt", 
           sep = "  ",
          row.names = FALSE)

out_shp <- in_riceatlas_shp %>% 
  left_join(final_out, by = c("OBJECTID" = "OBJECTID", "ISO" = "cty"))

write_shape(out_shp,
            "/home/lb/my_data/prasia/Data/vector/RiceAtlas_reshuffled.shp")
