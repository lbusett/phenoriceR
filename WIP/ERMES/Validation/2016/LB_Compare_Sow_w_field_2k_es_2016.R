...
# spTransform(es_shape_14,CRS(proj4string(crop_rast_14)))
#
...
es_shape_14@data$sowing_met = c("Water")
...
# prodry_142k = ZonalPipe(es_grid_sinu,raster_obs_14_dry,rast_grid, 'pippo',
#                       'int_id', stat="mean", cut = F)
# 
# prodry_142k_count = ZonalPipe(es_grid_sinu,raster_obs_14_dry,rast_grid, 'pippo',
#                             'int_id', stat="countna", cut = F)

prowat_142k = ZonalPipe(es_grid_sinu,raster_obs_wat_14,rast_grid, 'pippo',
                      'int_id', stat="mean", cut = F)

prowatcount_142k = ZonalPipe(es_grid_sinu,raster_obs_wat_14,rast_grid, 'pippo',
                           'int_id', stat="countna", cut = F)
...
# spTransform(es_shape_15,CRS(proj4string(crop_rast_15)))
#
es_grid_2k = readOGR(dsn = dirname(es_grid_reduced),layer = basename(file_path_sans_ext(es_grid_reduced)))
es_grid_25_sinu = spTransform(es_grid_2k,CRS(proj4string(es_rast_15)))
writeOGR(es_grid_25_sinu, dsn = Accessory_folder,layer  ='grid_2k_sinu', driver = 'ESRI Shapefile',overwrite_layer = T)

es_shape_15@data$sowing_met = c("Water")
...
# es_shape_15@data$sowing_met[which(es_shape_15@data$sowing_met == 'Unknown' & es_shape_15@data$sowing_doy< 120)] = 'Dry'
# es_shape_15@data$sowing_met[which(es_shape_15@data$sowing_met == 'Unknown' & es_shape_15@data$sowing_doy >= 120)] = 'Water'
...
# prodry_152k = ZonalPipe(es_grid_sinu,raster_obs_15_dry,rast_grid, 'pippo',
#                         'int_id', stat="mean", cut = F)
# 
# prodry_152k_count = ZonalPipe(es_grid_sinu,raster_obs_15_dry,rast_grid, 'pippo',
#                               'int_id', stat="countna", cut = F)

prowat_152k = ZonalPipe(es_grid_sinu,raster_obs_wat_15,rast_grid, 'pippo',
                        'int_id', stat="mean", cut = F)

prowatcount_152k = ZonalPipe(es_grid_sinu,raster_obs_wat_15,rast_grid, 'pippo',
                             'int_id', stat="countna", cut = F)
...
# spTransform(es_shape_16,CRS(proj4string(crop_rast_16)))
#
es_grid_2k = readOGR(dsn = dirname(es_grid_reduced),layer = basename(file_path_sans_ext(es_grid_reduced)))
es_grid_25_sinu = spTransform(es_grid_2k,CRS(proj4string(es_rast_16)))
writeOGR(es_grid_25_sinu, dsn = Accessory_folder,layer  ='grid_2k_sinu', driver = 'ESRI Shapefile',overwrite_layer = T)

es_shape_16@data$sowing_met = c("Water")
crop_rast_16 = crop(es_rast_16, extent(es_grid_25_sinu))
es_shape_16@data$sowing_met[which(is.na(es_shape_16@data$sowing_met))] = 'Unknown'
# es_shape_16@data$sowing_met[which(es_shape_16@data$sowing_met == 'Unknown' & es_shape_16@data$sowing_doy< 120)] = 'Dry'
# es_shape_16@data$sowing_met[which(es_shape_16@data$sowing_met == 'Unknown' & es_shape_16@data$sowing_doy >= 120)] = 'Water'
...
pro_162k = ZonalPipe(es_grid_sinu,crop_rast_16,rast_grid,'pippo',
                     'int_id', stat="mean", cut = T)

# prodry_162k = ZonalPipe(es_grid_sinu,raster_obs_16_dry,rast_grid, 'pippo',
#                         'int_id', stat="mean", cut = F)
# 
# prodry_162k_count = ZonalPipe(es_grid_sinu,raster_obs_16_dry,rast_grid, 'pippo',
#                               'int_id', stat="countna", cut = F)

prowat_162k = ZonalPipe(es_grid_sinu,raster_obs_wat_16,rast_grid, 'pippo',
                        'int_id', stat="mean", cut = F)

prowatcount_162k = ZonalPipe(es_grid_sinu,raster_obs_wat_16,rast_grid, 'pippo',
                             'int_id', stat="countna", cut = F)

proall_162k = ZonalPipe(es_grid_sinu,raster_obs_16,rast_grid, 'pippo',
                        'int_id', stat="mean", cut = F)

proallcount_162k = ZonalPipe(es_grid_sinu,raster_obs_16,rast_grid, 'pippo',
                             'int_id', stat="countna", cut = F)                             
...
names(protot_142k) = c(names(pro_142k@data)[1:10], 'Mod' , 'FieldTot', 'FieldWat','counttot','countwat')
...
protot_melt_142k = melt(profull_142k, measure.vars = c('Mod' , 'FieldTot',  'FieldWat', 'difftot','diffwat'))
...
names(protot_152k) = c(names(pro_152k@data)[1:10], 'Mod' , 'FieldTot',  'FieldWat','counttot','countwat')
...
protot_melt_152k = melt(profull_152k, measure.vars = c('Mod' , 'FieldTot',  'FieldWat', 'difftot','diffwat'))
...
names(protot_162k) = c(names(pro_162k@data)[1:10], 'Mod' , 'FieldTot',  'FieldWat','counttot','countwat')
...
protot_melt_162k = melt(profull_162k, measure.vars = c('Mod' , 'FieldTot',  'FieldWat', 'difftot','diffwat'))
...
protot_melt_14_15_162k = melt(profull_14_15_162k, measure.vars = c('Mod' , 'FieldTot',  'FieldWat', 'difftot','diffwat'))
...
es_2k = profull_14_15_162k
es_2k_melt  = protot_melt_14_15_162k
es_stats_14_15_16 = ddply(subset(es_2k_melt),.(variable) ,summarize, count = length(value),avg = mean(value, na.rm = T), stdev = sd(value, na.rm = T), mae = mean(abs(value),na.rm = T))
save(es_2k,es_2k_melt,es_stats_14_15_16, file = file.path(out_folder,'es_val_2k.RData'))
