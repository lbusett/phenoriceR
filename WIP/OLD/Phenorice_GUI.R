...
#- ------------------------------------------------------------------------------- -#
{{infold_frame <- gframe(text = '<span foreground="blue" size="large">Input Folders and Files</span>', markup = T, container=main_group, expand = T,spacing = 15, horizontal = F)    			# Frame group
...
{{outfold_frame <- gframe(text = '<span foreground="blue" size="large">Output Folder for Results storage</span>', markup = T, container=main_group, expand = T,spacing = 15)    			# Frame group
...
{{seas_frame <- gframe(text = '<span foreground="blue" size="large">Typical Heading Periods</span>', markup = T, container=main_group, expand = T,spacing = 15, horizontal = F)    			# Frame group
...
#		
		...
		start_day_wid_1   <- gspinbutton(1,31,  container=seas1_group , value = general_opts$seas1_start_dayprogr)
		seas1_lab <- glabel(text = 'Start Month',markup = T, container = seas1_group)
		start_month_wid_1 <- gspinbutton(1,12,  container=seas1_group , value = general_opts$seas1_start_month)
		...
		end_month_wid_1 <- gspinbutton(1,12,  container=seas1_group , value = general_opts$seas1_end_month)
		addSpace(seas1_group, 25, horizontal=TRUE)
...
years_frame = gframe(text = '<span foreground="blue" size="large">Select Start and End Years</span>', markup = T, container=main_group, expand = T,spacing = 15)    			# Frame group
...
#										lst_check = 'On' ,lst_thresh = 15)
							...
							general_opts$seas1_end_doy <- as.integer(strftime(today, format = "%j"))
#								browser()
