...
# Define function
Phenorice_aggr_adm <- function(country_code, gadm_level, phenorice_path, main_dir, aggr_stats, csv_filename=NA, xlsx_filename=NA , rdata_outfilename = NA) {
	...
	save(phenorice_dt, aggr_dt_N_Seasons, aggr_dt, file = rdata_outfilename)
...
phenorice_path = '//10.0.1.252/nr_working/shared/PhenoRice/Processing/Senegal/Outputs/new_Elab2'

# Indicate the directory path where files (gadm shapes and rasters, and eventually xlsx and/or csv output) have to be saved
outputs_folder = "D:/Temp/PhenoRice/Processing/SEN/Outputs/new_Elab2/"
...
outputs_folder = "D:/Temp/PhenoRice/Processing/SEN/Outputs/new_Elab2/"		# Folder where to put results of this processing
...
for (year in years) {
  ...
  # get in filename
  phrice_outfolder = file.path(phenorice_path,year,'raster')
  phenorice_path_year = list.files(phrice_outfolder, glob2rx('*Full_Out*dat'),full.names  = TRUE)

  # sset output folder
  year_out_folder = paste0(outputs_folder,year)
  rdata_outfilename = file.path(year_out_folder,paste0('out_df_modgrid_',year,'.RData'))
  dir.create(dirname(rdata_outfilename), recursive = T)

  #Launch
phenorice_aggr_stats <- Phenorice_aggr_adm( country_code = country_code, gadm_level = gadm_level,
		phenorice_path = phenorice_path_year[2], main_dir = outputs_folder, aggr_stats = aggr_stats,
		xlsx=xlsx_filename, csv=csv_filename ,rdata_outfilename = rdata_outfilename)
}

# for (year in years) {

  phenorice_plots = LB_Phenorice_plot_dates_Sen(start_year, end_year, outputs_folder)
# }
