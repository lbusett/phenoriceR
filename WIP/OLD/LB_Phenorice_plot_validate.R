...
results_folder = file.path(main_folder,'Validate/WS/max_4000_min_3000')			# Where to put results
sel_seasons = 'Second'
sel_quarts = c(1)
...
in_phrice_file = list.files("//10.0.1.252/nr_working/shared/PhenoRice/Processing/PHL/Outputs/2013/raster", glob2rx('*Full_Out*dat'),full.names  = TRUE)
...
#x11()
#plot(refmap, col = c('transparent','red'), colNA = 'grey75', legend = FALSE, xlim=c(xmin(refmap), xmax(refmap)), ylim=c(ymin(refmap), ymax(refmap)))
#legend(x='bottomleft', legend = c("Not Rice","Rice", "NoData"), fill = c('transparent','red','grey75'))
#plot(in_shape_admin_reproj, add = T)
#
#spplot(refmap, maxpixels = ncell(refmap))+layer(sp.polygons(in_shape_admin_reproj))


#x11()
#mod_map = raster(in_phrice_file, band = 2)
#
#mod_map[(mod_map != 0 & mod_map != 365)] = 1
#mod_map[mod_map == 0] = NA
#mod_map[mod_map > 365] = NA
#spplot(mod_map)
#
#ref_map = raster(refmap_file)
#ref_map[ref_map == 0] = NA
#spplot(ref_map)
...
#			acc_mat_areas = rbind(acc_mat_areas, acc_areas_seas)
			acc_mat_areas_Admin = dlply(in_modgrid_df, .(NAME_1), function(df)acc_areas_comp(df,Season))		# Compute separated by province
...
# Plot Area Comparisons on all map
		...
		p_area_comp_all = p + ggtitle("MODIS vs Reference Area")
...
# Plot Accracies on all mop and then by province
		...
		p_accuracy = p+ theme(legend.justification=c(1,0), legend.position="top")+ ggtitle("Detection Accuracies (Computed on Areas)")
...
{{
		...
		for (Season in sel_seasons) {
			...
			acc_pix_all$NAME_1 = 'All'
			acc_pix_admin =  ddply(in_modgrid_df, .(NAME_1), function(df)acc_pixels(df,Season))   # by province
...
p = ggplot (droplevels(subset(plot_data,NAME_1 =='All' & variable == 'Omission_Err_Rice' )),aes(x = threshold, y = 100-value, pch = variable, colour = Season, group = interaction(Season,variable))) + theme_bw()
...
p_detection = p+ggtitle('Detection rate (a.k.a. producers accuracy) as a function of rice fc in MODIS pixels')
...
p = p +	scale_x_continuous('% of rice in MODIS pixel', limits = c(0,100), breaks = seq(0,100,10) )+
		scale_y_continuous('Metric (%)', limits = c(0,100),breaks = seq(0,100,10))+theme(plot.title = element_text(vjust = 1, hjust = 0))
p_detection_pro = p+ggtitle('Omission and Commision (as % of pixels) as a function of rice fc in MODIS pixels - By Provinces')
...
#	grid_res =
	...
	for (quart in sel_quarts) {
		plot_index = plot_index +1
		yvar = yvars[quart]
		x = aggregate_values$ref_rice_Area
		y = aggregate_values[[yvar]]
		Regr_Stats =  regress_stats( x, y )
		eqn_strings = lm_eqn(Regr_Stats)
		ref_rice_Area = 'ref_rice_Area'
#	eqn_strings$range_max = range_max

		aggregate_values$mod_ricearea_q6 = aggregate_values$mod_ricearea_q3 + aggregate_values$mod_ricearea_q4
		p1 = ggplot(data = aggregate_values, aes_string(x = ref_rice_Area, y = 'mod_ricearea_q6'))
...
# 	browser()
	...
	maps_modis [[plot_index]] = mapfract_mod
...
save(p_area_comp_all,p_area_comp_prov, p_accuracy, p_accuracy_pro,
     p_detection, p_detection_pro, plots_accuracy, maps_sar,
     maps_modis, file = out_RData_file)
