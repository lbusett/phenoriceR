...
es_grid = '/media/projects/ermes/datasets/rs_products/Phenology/Validation/2016/ES/Accessory/ES_ERMES_Regional_Grid.shp'


es_shape_16 = readOGR (dirname(es_shape_file_16), file_path_sans_ext(basename(es_shape_file_16)))
es_shape_16@data$user_id = NULL
es_rast_16 = raster(es_raster_file_16, band = 2)
es_2km_16 = raster(es_2km_tif_16)
es_grid_sp = readOGR(dirname(es_grid), file_path_sans_ext(basename(es_grid)))
...
es_ras_fields_16 = extract(es_rast_16,es_shape_16,weights=TRUE,fun = mean, small = T,  na.rm = T, df = T)

save(es_ras_fields_16, file = '/media/projects/ermes/datasets/rs_products/Phenology/Validation/2016/ES/ES_16.RData')
#load('d:/temp/phenorice/processing/Validation_Ermes/ES_16.RData')
# ras_fields_stdev = extract(in_rast,in_shape,fun = sd, na.rm = T, df = T)
es_ras_fields_16$parcel_id = es_shape_16@data$parcel_id
joined_16 = join(es_shape_16@data, es_ras_fields_16, type = 'left')
...
save(es_ras_fields_15, file = '/media/projects/ermes/datasets/rs_products/Phenology/Validation/2016/ES/ES_15.RData')
#load('d:/temp/phenorice/processing/Validation_Ermes/ES_15.RData')
# ras_fields_stdev = extract(in_rast,in_shape,fun = sd, na.rm = T, df = T)
es_ras_fields_15$parcel_id = es_shape_15@data$parcel_id
joined_15 = join(es_shape_15@data, es_ras_fields_15, type = 'left')
...
save(es_ras_fields_14, file = '/media/projects/ermes/datasets/rs_products/Phenology/Validation/2016/ES/ES_14.RData')
# ras_fields_stdev_14 = extract(es_rast,es_shape_14,fun = sd, na.rm = T, df = T)
load('/media/projects/ermes/datasets/rs_products/Phenology/Validation/2016/ES/ES_14.RData')
es_ras_fields_14$parcel_id = (es_shape_14@data$parcel_id)
...
es_joined_14$sowing_doy = as.numeric(as.character(es_joined_14$sowing_doy))
es_joined_14$diff = es_joined_14$Sow_MOD - es_joined_14$sowing_doy

sub_14 = droplevels(subset(es_joined_14, crop_type =='Rice' & is.na(sowing_doy) ==FALSE & Sow_MOD !=0 ))
...
es_or = sub_tot
es_or_melt  = joinedmelt_tot
save(es_or_melt,es_or, file = '/media/projects/ermes/datasets/rs_products/Phenology/Validation/2016/ES/ES_val_or.RData')
...
stats = ddply(subset(es_or_melt, area > 2), .(sowing_met,variable,year) ,summarize, count = length(value), avg = mean(value, na.rm = T), stdev = sd(value, na.rm = T), mae = mean(abs(value),na.rm = T),sderr = std (value))
...
p = ggplot(droplevels(subset(es_or_melt, variable !='diff'& area > 1)), aes(x = value, fill = variable ))
