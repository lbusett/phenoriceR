...
phenorice_path = '//10.0.1.252/nr_data/raster_products/Phenology/PHL/Outputs/v1.0/2013/raster/old_min_identification_Full_Out_2013_masked.dat'

phenorice_path = '//10.0.1.252/nr_working/lorenzo/phenorice/India/Outputs/Seasons_dist_32_max_4000_all/2014/raster/2014/raster/old_min_identification_Full_Out_2014.dat'

# Indicate the directory path where files (gadm shapes and rasters, and eventually xlsx and/or csv output) have to be saved
main_dir = 'D:/Temp/stats_India'

# Indicate the name of the xlsx output file (without the extension), or leave NA not to generate it.
xlsx_filename = 'prova.xlsx'
...
Phenorice_aggr_adm <- function(country_code, gadm_level, phenorice_path, main_dir, aggr_stats, csv_filename=NA, xlsx_filename=NA ) {
	...
	aggr_dt_N_Seasons = phenorice_dt[,list(
					...
					n_isrice   = sum(N_Seasons>0,na.rm = T),
					n_1season  = sum(N_Seasons==1,na.rm = T),
					n_2seasons = sum(N_Seasons==2,na.rm = T),
					n_3seasons = sum(N_Seasons==3,na.rm = T),
					n_4seasons = sum(N_Seasons==4,na.rm = T),
