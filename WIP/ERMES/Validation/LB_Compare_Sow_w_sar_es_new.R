es_shape_file_15 = 'D:/Temp/PhenoRice/Processing/Validation_Ermes/Input/ES_Static_info_2015_buf.shp'
es_shape_15 = readOGR (file.path(dirname(es_shape_file_15)), file_path_sans_ext(basename(es_shape_file_15)))
type = "DSC"

if (type == "DSC" ){
  in_raster = raster('D:/Temp/PhenoRice/Processing/Validation_Ermes/ES/SAR/IS_SoS')
  in_raster2 =raster( 'D:/Temp/PhenoRice/Processing/Validation_Ermes/ES/SAR/IS_PoS')
  out_shp = 'D:/Temp/PhenoRice/Processing/Validation_Ermes/Input/ES_Static_info_2015_w_sar_DSC.shp'
  out_rdata = 'd:/temp/phenorice/processing/Validation_Ermes/ES_15_SAR_Dsc.RData'
  values(in_raster) = (values(in_raster)-2)*12+113
  values(in_raster)[which(values(in_raster) < 113)] =NA
  values(in_raster2) = (values(in_raster2)-2)*12+113
  values(in_raster2)[which(values(in_raster2) < 113)] =NA
  small_15 = crop(in_raster, es_shape_15)
  small_15_pos = crop(in_raster2, es_shape_15)
  dir.create('D:/Temp/PhenoRice/Processing/ES/ancillary/validation_maps/SAR/DSC/')
  writeRaster(small_15,'D:/Temp/PhenoRice/Processing/ES/ancillary/validation_maps/SAR/DSC/SOS_Small_DSC.tif', overwrite = TRUE)
  writeRaster(small_15_pos,'D:/Temp/PhenoRice/Processing/ES/ancillary/validation_maps/SAR/DSC/POS_Small_DSC.tif', overwrite = TRUE)
}
...
shp = ZonalPipe(es_shape_file_15,
           small_15,
...
writeOGR(shp, dirname(out_shp), layer= sub("^([^.]*).*", "\\1", basename(out_shp)), driver="ESRI Shapefile", overwrite_layer = T)
 ...
 path.in.shp =  'D:/Temp/PhenoRice/Processing/Validation_Ermes/Input/ES_Static_info_2015_w_sar_All.shp'
 shp<-readOGR(path.in.shp, layer= sub("^([^.]*).*", "\\1", basename(path.in.shp)))
 names(shp@data)[17:26] = c("avg_sos","pixcnt1","maj_sos","z1","avg_pos","pixcnt","maj_pos","erravg","psss_diff","errmaj")
 data = shp@data
 ...
 path.in.shp =  'D:/Temp/PhenoRice/Processing/Validation_Ermes/Input/ES_Static_info_2015_w_sar_DSC.shp'
 shp<-readOGR(path.in.shp, layer= sub("^([^.]*).*", "\\1", basename(path.in.shp)))
 shp@data$type = 'DSC'
 names(shp@data)[17:26] = c("avg_sos","pixcnt1","maj_sos","z1","avg_pos","pixcnt","maj_pos","erravg","psss_diff","errmaj")
 # data = rbind(data, shp@data)
